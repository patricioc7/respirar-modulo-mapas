  ...
  const handleSearch = () => {
    if (searchWords) {
      const filteredStations = stations.filter((station) =>
        station.name.toLowerCase().includes(searchWords.toLowerCase())
      );
          ...
          foundStations.map((station) => {
            return (
              <div>
                <Button
                  className="boton"
                  onClick={() => handleResultClick(station.coordinates)}
                >
                  {" "}
                  <Image src={icon} className="pinResultado" /> {station.name}{" "}
                </Button>
              </div>
            );
          })}
